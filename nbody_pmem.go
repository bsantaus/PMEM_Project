...
func NewSystem(body []Body) System {
   txn("undo") {
      var n System
      n = pmem.Make("newsys", n, len(body)).(System)
      for i := 0; i < len(body); i++ {
         n[i] = (*Body)(pmem.New("body" + strconv.Itoa(i), n[i])) // copy to avoid overwriting the inputs
      }
      return resetSystem(n, body)
   }
}
...
func main() {
   ...
   body := []Body{sun, jupiter, saturn, uranus, neptune}
   if firstInit {
      system = NewSystem(body)
   } else {
      system = pmem.GetSlice("newsys", system).(System)

      if system == nil {
         system = NewSystem(body)
      } else {
         txn("undo") {
            system = resetSystem(system, body)
         }
      }
   }
